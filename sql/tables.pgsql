...
CREATE TYPE attempt_status AS ENUM ('INITIALIZATION','PRE_CLEANUP','FETCH_INPUT','RUN','WRITE_OUTPUT','POST_HEALTHCHECK','POST_CLEANUP','EXIT');
...
CREATE INDEX ON attempt (role_id);                                 -- for finding the attempts linked to a role (not used by the current code)
